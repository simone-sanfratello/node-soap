 ...
 * @param {String} type
 */
WSDL.prototype.objectToDocumentXML = function(name, params, nsPrefix, nsURI, type, firstPrefix) {
  var args = {};
  args[name] = params;
  var parameterTypeObj = type ? this.findSchemaObject(nsURI, type) : null;
  return this.objectToXML(args, null, nsPrefix, nsURI, true, null, parameterTypeObj, null, firstPrefix);
};
 ...
 */
WSDL.prototype.objectToXML = function(obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext, firstPrefix) {
  var self = this;
  var schema = this.definitions.schemas[nsURI];
  ...
  if (Array.isArray(obj)) {
    ...
    for (name in obj) {
      ...
      if (prefixNamespace && !qualified && isFirst && !self.options.overrideRootElement) {
        ...
        ns = '';
      } else if (isFirst && firstPrefix) {
        ns = firstPrefix
      }

      if(value || !self.options.useEmptyTag) {
        if (!Array.isArray(child)) {
          // start tag
        ...
        if (!Array.isArray(child)) {
          ...
          parts.push(['<', emptyNonSubNameSpace ? '' : appendColon(nonSubNameSpace || ns), name, attr, xmlnsAttrib, '/>'].join(''));
      }
    }
